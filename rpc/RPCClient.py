...
class RPCClient:
    ...
    def __init__(self, host, port, dataPacketFormat = "bson",timeout = 10, handler = None):
        ...
        self.dataPacketFormat = dataPacketFormat
         
    def request(self, func, params):
        self.seq += 1
        d = {}
        if self.dataPacketFormat == "json":
            d = {u"rpcname":func, u"rpcparams":params}
        else:
            d = {u"func":func, u"params":params}
        sendPack = TTPackage(self.seq, NET_CMD_RPC, d)
        datas = sendPack.encode_withFormat(self.dataPacketFormat)
    ...
    def _recvOnePackage(self):        
        ...
        package = TTPackage()
        package.decode_withFormat(datas, self.dataPacketFormat)
        return package

def request(address, func, param):
    pos = address.find(":")
    ip = address[0:pos]
