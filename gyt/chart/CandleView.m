...
- (void)initView {
    ...
    //candleChart
    self.candleChart = [[Chart alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
    self.candleChart.userInteractionEnabled = YES;
...
-(void)initChart{
    ...
    if(indicatorsString != nil){
        NSMutableArray *indicators = [NSMutableArray mj_objectArrayWithKeyValuesArray:indicatorsString];
    ...
    if (self.chartMode == 0) {
        if([self.req_type isEqualToString:@"T"]){
            ...
            if([self.req_freq hasSuffix:@"m"]){
                self.req_type = @"L";
                self.timer = [NSTimer scheduledTimerWithTimeInterval:60 target:self selector:@selector(getData) userInfo:nil repeats:YES];
            }
        }else{
